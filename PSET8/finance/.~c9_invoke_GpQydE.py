...
def index():
    ...
    x = db.execute("SELECT * FROM holdings WHERE userid=:usrid", usrid=session['user_id'])
    # Determines amount of cash
    cash = db.execute("SELECT cash FROM users WHERE id=:sessionid", sessionid=int(session["user_id"]))[0]["cash"]
    ...
    for stock in x:
        ...
        transactions.append({
            ...
            "shares": stock['shares'],
            "price": "%.2f" % (look['price']),
            "total": "%.2f" % (look['price'] * stock['shares'])
        })
    # Render
    return render_template("portfolio.html", index=transactions, cash="%0.2f" % cash)
...
def buy():
    ...
    if request.method == "GET":
        ...
        db.execute("""
            ...
            SET cash = :cashamnt
            WHERE id = :sessionid
        """, cashamnt=cash, sessionid=int(session["user_id"]))
        ...
        """, userid=int(session["user_id"]), symbol=symbol["symbol"], shares=shares, price=symbol['price'], time=date)

        # Checks the database to see if the current stock already has a holding
        amnt_owned = db.execute("SELECT shares FROM holdings WHERE userid = :usrid AND symbol = :symbol", usrid=session['user_id'], symbol=symbol['symbol'])
        if (amnt_owned):
            # Update the amount of stock that is owned by the user
            amnt_owned = amnt_owned[0]['shares'] + int(shares)
            print(amnt_owned)
            db.execute("UPDATE holdings SET shares = :shares WHERE symbol = :symbol AND userid = :usrid", shares=amnt_owned, symbol=symbol['symbol'], usrid=session['user_id'])
        # If we don't already have a holding, create one
        else:
            db.execute("INSERT INTO holdings (userid, symbol, shares) VALUES (:usrid, :symbol, :shares)", usrid=session['user_id'], symbol=symbol['symbol'], shares=int(shares))
...
def register():
    ...
    if request.method == "POST":
        ...
        db.execute("INSERT INTO users (username, hash) VALUES (:user, :hashed)", user=username, hashed=generate_password_hash(password))
...
def sell():
    ...
    if request.method == "POST":
        ...
        current_share_amnt = db.execute("SELECT shares FROM holdings WHERE userid=:usrid AND symbol=:sym", usrid=session["user_id"], sym=stock['symbol'])[0]['shares']
        ...
        if final_shares == 0:
            ...
            db.execute("DELETE FROM holdings WHERE userid=:usrid AND symbol=:symbol", usrid=session["user_id"], symbol=stock['symbol'])
        else:
            # Updated stock amount
            db.execute("UPDATE holdings SET shares=:finalshares WHERE userid=:usrid AND symbol=:symbol", finalshares=final_shares, usrid=session['user_id'], symbol=stock['symbol'])

        # Updated cash amount with sale
        cash += shares * stock['price']
        # Update cash in database
        db.execute("UPDATE users SET cash = :newcash WHERE id = :usrid", newcash=cash, usrid=session['user_id'])
        ...
        x = db.execute("SELECT symbol FROM holdings WHERE userid=:usrid", usrid=session['user_id'])
        # Fills a list
        stocks = []
        for stock in x:
